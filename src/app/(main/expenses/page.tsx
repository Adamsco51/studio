  ...
  return (
    <>
      ...
      <Dialog open={showEditExpenseDialog} onOpenChange={(open) => {
        if (!open) {
            setExpenseTargetedForAction(null);
            if (pinActionType === 'edit') setActivePinRequest(null);
        }
      ...
      <AlertDialog open={showReasonDialog} onOpenChange={(isOpen) => {
        ...
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>{isAdmin ? `Supprimer Dépense: "${expenseTargetedForAction?.label}"?` : `Demande de Suppression: ${expenseTargetedForAction?.label}`}</AlertDialogTitle>
            {isAdmin ? (
                <AlertDialogDescription>
      ...
      <Dialog open={showPinDialog} onOpenChange={(isOpen) => {
        ...
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            ...
            <DialogDescription>
              Un PIN vous a été fourni par un administrateur pour {pinActionType === 'edit' ? 'modifier' : 'supprimer'} la dépense "{expenseTargetedForAction?.label || "sélectionnée"}".
            </DialogDescription>
          </DialogHeader>
